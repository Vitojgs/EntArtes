...
import { mockPedidosAulas, mockAnuncios, mockTurmas } from '../data/mockData';
...
export function Dashboard() {
  ...
  const getFilteredAulas = () => {
    if (user.role === 'ALUNO') return mockPedidosAulas.filter(a => a.alunoId === user.id);
    if (user.role === 'PROFESSOR') return mockPedidosAulas.filter(a => a.professorId === user.id);
    if (user.role === 'ENCARREGADO') return mockPedidosAulas.filter(a => a.encarregadoId === user.id);
    return mockPedidosAulas;
  };

  const allAulas = getFilteredAulas();

  const getFilteredAnuncios = () => {
    if (user.role === 'DIRECAO') return mockAnuncios;
    if (user.role === 'ENCARREGADO') return mockAnuncios.filter(a => a.vendedorId === user.id);
    return [];
  };
