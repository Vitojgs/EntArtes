...
|------|-------|-----------------|-------------|-------------|
| RF (Funcionais) | 34 | 34 | 0 | 0 |
| RNF (Não Funcionais) | 12 | 12 | 0 | 0 |
| **Total** | **46** | **46** | **0** | **0** |

**Cobertura global: 100% (46/46) totalmente conforme.**
...
| **RF03b** | Gestão de Múltiplas Roles | UC01.04 | ✅ | Backend: `auth.service.js` detecta múltiplas roles via queries às tabelas perfil (direcao, professor, encarregadoeducacao, aluno). Retorna `availableRoles` no login. Frontend: `DashboardLayout.tsx` com role switcher dropdown, `AuthContext.tsx` com `activeRole`. `role.middleware.js` — `authorizeRole()` para autorização granular. `tokenVersion` para invalidação de todas as sessões ativas. Testado na sessão 2026-05-06. |
...
|-----|------|-----------|--------|-----------|
| **RNF01** | Segurança de Credenciais | Segurança | ✅ | Palavras-passe armazenadas com bcrypt (`hashSync`/`compareSync`). Nunca armazenadas em texto simples. JWT com segredo configurável via `.env`. **Sistema de invalidação de sessões**: incremento de `tokenVersion` na tabela `utilizador` invalida imediatamente todos os tokens ativos forçando re-login. Controlo de estado: utilizador com `estado=false` é rejeitado em todas as requests com 401 "Utilizador desativado". |
| **RNF02** | Controlo de Acesso | Segurança | ✅ | 5 perfis: `ALUNO`, `ENCARREGADO`, `PROFESSOR`, `DIRECAO`, `UTILIZADOR`. Middleware `verifyToken` em `onRequest` das rotas protegidas. `hasRole(role, ...allowed)` verificado em cada endpoint. Suporte a múltiplas roles por utilizador com `availableRoles` e role switcher no frontend. `role.middleware.js` — `authorizeRole()` para autorização baseada em roles permitidas. |
